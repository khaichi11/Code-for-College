...
import random # Ditambahkan untuk partikel decay (opsional)
...
DARK_METAL = (80, 80, 90)
WATER_BLUE = (100, 150, 255, 120) # Added alpha
...
class DCMotor:
    ...
    def update(self, voltage, dt):
        ...
        if self.is_feeding:
            ...
            if angle_error < 2.0 and abs(self.angular_velocity) < 5:
                self.feed_complete = True
                self.is_feeding = False
...
class FeedingSystem:
    ...
    def dispense_feed(self, volume, motor_angle):
        particle_count = max(1, int(volume * 10000))
    ...
    def update_particles(self, tank_rect, water_surface_y):
        tank_bottom = tank_rect.bottom - 5 # Padded bottom

        for particle in self.feed_particles[:]:
            if particle['eaten']:
                try:
                    self.feed_particles.remove(particle)
                except ValueError:
                    pass
                continue
            ...
            if not particle['settled']:
                ...
                if particle['y'] >= tank_bottom - particle['size'] / 2:
                    particle['y'] = tank_bottom - particle['size'] / 2
                    particle['vy'] = 0
                    particle['vx'] = 0
                    particle['settled'] = True
                    particle['life'] = 5000 # Longer life for settled, more time to eat

                particle['x'] = max(tank_rect.left + 5, min(tank_rect.right - 5, particle['x']))

            particle['life'] -= 1
            # Only remove if life ends *and* it's NOT settled
...
class FishFeederSimulator:
    def __init__(self):
        ...
        pygame.display.set_caption("Enhanced Fish Feeder Simulation V3")
        ...
        # Updated button definitions - will be set in draw_control_panel
        self.buttons = {}

        self.font = pygame.font.Font(None, 24)
        self.title_font = pygame.font.Font(None, 36)
        self.small_font = pygame.font.Font(None, 18)
        self.button_font = pygame.font.Font(None, 20) # Slightly smaller button font
    ...
    def _initialize_fish(self):
        ...
        for i in range(self.num_fish):
            self.fish_list.append({
                ...
                'vx': np.random.uniform(0.5, 1.2) * np.random.choice([-1, 1]), 'vy': 0,
                ...
                if event.button == 1:
                    ...
                    for name, rect in self.buttons.items():
                        if rect.collidepoint(pos):
                            ...
                            break # Only process one button click
    ...
    def reset_system(self):
        ...
        self.target_angle_history.clear()
    ...
    def _update_fish_behavior(self):
        ...
        for fish in self.fish_list:
            ...
            if fish['target_particle'] and (fish['target_particle']['eaten'] or fish['target_particle'] not in self.feeding_system.feed_particles):
                 fish['target_particle'] = None
                 fish['state'] = 'swimming'

            if not fish['target_particle']:
                closest_particle_found = None
                min_dist_sq_found = search_radius_sq

                for p in available_particles:
                    is_targeted = False
                    for other_fish in self.fish_list:
                        if other_fish['target_particle'] == p:
                            is_targeted = True
                            break
                    if is_targeted: continue

                    dx = p['x'] - fish_center_x
                    dy = p['y'] - fish_center_y
                    dist_sq = dx*dx + dy*dy
                    if dist_sq < min_dist_sq_found:
                        min_dist_sq_found = dist_sq
                        closest_particle_found = p

                if closest_particle_found:
                    fish['target_particle'] = closest_particle_found
                    fish['state'] = 'chasing'
                else:
                    fish['state'] = 'swimming'

            if fish['state'] == 'chasing' and fish['target_particle']:
                target = fish['target_particle']
                dx = target['x'] - fish_center_x
                dy = target['y'] - fish_center_y
                dist_sq = dx*dx + dy*dy
                ...
                if dist_sq < eat_distance_sq:
                    ...
                    for f in self.fish_list:
                        if f['target_particle'] == target:
                            f['target_particle'] = None
                            f['state'] = 'swimming'
                else:
                    dist = math.sqrt(dist_sq) if dist_sq > 0 else 1
                    fish['vx'] = (dx / dist) * chase_speed_x
                    fish['vy'] = (dy / dist) * chase_speed_y

            elif fish['state'] == 'swimming':
                fish['vy'] = np.sin(time.time() * 1.5 + fish['id']) * 0.4
                if abs(fish['vx']) < 0.3:
                   fish['vx'] = np.random.uniform(0.5, 1.2) * np.random.choice([-1, 1])
            ...
            fish['x'] = max(self.tank_rect.x + 5, min(self.tank_rect.right - fish['width'] - 5, fish['x'])) # Adjusted boundary
            fish['y'] = max(self.water_surface_y + 5, min(self.tank_rect.bottom - fish['height'] - 5, fish['y']))

            if fish['x'] <= self.tank_rect.x + 5 or fish['x'] >= self.tank_rect.right - fish['width'] - 5:
                fish['vx'] *= -1
                fish['state'] = 'swimming'
                fish['target_particle'] = None
    ...
    def update_system(self):
        ...
        if self.system_active:
            if self.motor.is_feeding:
                ...
                voltage = 0
                if abs(self.motor.angular_velocity) > 0.1 : voltage = -self.motor.angular_velocity * 0.05
                else: self.motor.angular_velocity = 0
        else:
            voltage = 0
            self.motor.angular_velocity *= 0.95
            if abs(self.motor.angular_velocity) < 0.1: self.motor.angular_velocity = 0

        self.motor.update(voltage, dt)
        self.motor_shaft_angle = self.motor.current_angle
            ...
            if current_feed_rate > 0.00001:
                ...
                if np.random.random() < 0.3:
                    self.water_ripples.append({
                        'x': 500 + np.random.randint(-15, 15),
                        'y': self.water_surface_y + np.random.randint(-2, 2),
                        'radius': 3, 'alpha': 255 })
    ...
    def draw_motor_assembly(self):
        ...
        pygame.draw.line(self.screen, DARK_METAL, (center_x, center_y), (center_x + 25 * math.cos(shaft_angle_rad), center_y + 25 * math.sin(shaft_angle_rad)), 4)

        for i in range(3):
            blade_angle = shaft_angle_rad + i * (2 * math.pi / 3)
            pygame.draw.arc(self.screen, RED, (center_x - 25, center_y - 25, 50, 50), blade_angle - 0.3, blade_angle + 0.3, 4)
            support_x_start = center_x + 8 * math.cos(blade_angle - 0.1); support_y_start = center_y + 8 * math.sin(blade_angle - 0.1)
            support_x_end = center_x + 22 * math.cos(blade_angle); support_y_end = center_y + 22 * math.sin(blade_angle)
            pygame.draw.line(self.screen, ORANGE, (support_x_start, support_y_start), (support_x_end, support_y_end), 3)
    ...
    def draw_control_panel(self):
        pygame.draw.rect(self.screen, (230, 230, 230), (30, 30, 550, 230), border_radius=10) # Increased height
        pygame.draw.rect(self.screen, DARK_GRAY, (30, 30, 550, 230), 3, border_radius=10)
        self.screen.blit(self.font.render("CONTROL PANEL", True, DARK_GRAY), (40, 35))

        # Power Button
        self.buttons['power'] = pygame.Rect(50, 70, 100, 40) # Adjusted Y
        ...
        # Feed Buttons (Adjusted Layout)
        button_width = 120
        button_height = 40
        start_x = 50
        y_pos = 125 # Adjusted Y
        spacing_x = 10
        ...
        # Lower Buttons (Adjusted Layout)
        lower_y = 180 # Adjusted Y
        btn_w_low = 150
        space_low = 25
        start_x_low = 50
        ...
        self.screen.blit(self.small_font.render(pid_text, True, DARK_GRAY), (40, 235)) # Adjusted Y
    ...
    def draw_status_panel(self):
        ...
        col1_x = panel_x + 30
        col2_x = panel_x + panel_w // 2 + 15 # Adjusted X

        status_texts_col1 = [
            f"SYSTEM: {'ACTIVE' if self.system_active else 'INACTIVE'}",
            f"FEEDING: {'IN PROGRESS' if self.motor.is_feeding else 'IDLE'}",
            "",
            f"MOTOR ANGLE: {self.motor.current_angle:.1f}°",
            f"TARGET ANGLE: {self.motor.target_angle:.1f}°",
            f"MOTOR RPM: {self.motor.get_rpm():.1f}",
        ]
        status_texts_col2 = [
            f"VOLTAGE: {self.motor.voltage:.2f}V",
            f"CURRENT: {self.motor.current:.3f}A",
            "",
            f"FEED LEVEL: {self.feed_level:.0f}%",
            f"SESSIONS: {self.feeding_system.feed_sessions}",
            f"TOTAL DISP.: {self.feeding_system.total_feed_dispensed*1000:.1f}ml",
        ]

        start_y = panel_y + 65; line_height = 28 # Increased Y and height
    ...
    def draw_performance_graph(self):
        graph_x = 780
        status_panel_h = 320 # Updated height
        status_panel_y = 30
        graph_y = status_panel_y + status_panel_h + 20
        graph_w = SCREEN_WIDTH - graph_x - 20
        graph_h = SCREEN_HEIGHT - graph_y - 20 # Use remaining height
        ...
        for i in range(1, 4):
            y_grid = graph_rect.y + i * (graph_rect.height // 4)
            pygame.draw.line(self.screen, (220, 220, 220), (graph_rect.x, y_grid), (graph_rect.right, y_grid))
        for i in range(1, 6):
            x_grid = graph_rect.x + i * (graph_rect.width // 6)
            pygame.draw.line(self.screen, (220, 220, 220), (x_grid, graph_rect.y), (x_grid, graph_rect.bottom))
        ...
        if len(self.angle_history) > 1:
            pygame.draw.lines(self.screen, BLUE, False, get_points(self.angle_history), 2)
        if len(self.target_angle_history) > 1:
            pygame.draw.lines(self.screen, RED, False, get_points(self.target_angle_history), 2)
    ...
    def draw_feed_particles(self):
        for particle in self.feeding_system.feed_particles:
            ...
            if particle['shape'] == 'circle':
                ...
                ew, eh = size, max(1, size//2)
                s = pygame.Surface((ew, eh), pygame.SRCALPHA)
                pygame.draw.ellipse(s, (*particle['color'], alpha), (0, 0, ew, eh))
                self.screen.blit(s, (particle['x']-ew//2, particle['y']-eh//2))
    ...
    def draw_water_tank(self):
        ...
        # Draw Gravel (Only Base Color, No Dots)
        ...
        water_fill_rect = pygame.Rect(self.tank_rect.x + 3, self.water_surface_y + 3,
                                      self.tank_rect.width - 6, self.tank_rect.height - (self.water_surface_y - self.tank_rect.y) - 6)
        ...
        for fish in self.fish_list:
            ...
            is_flipped = fish['vx'] < 0
            fish_surf = pygame.Surface((w + 20, h + 5), pygame.SRCALPHA)
            body_x = 10 if not is_flipped else 5
            body_rect = pygame.Rect(body_x, 0, w - 10, h)

            tail_sway = math.sin(fish['tail_phase']) * 6
            tail_base_x = body_rect.left if not is_flipped else body_rect.right
            tail_tip_x = tail_base_x - 10 if not is_flipped else tail_base_x + 10
            tail_points = [
                (tail_base_x, body_rect.centery),
                (tail_tip_x, body_rect.top + tail_sway),
                (tail_tip_x, body_rect.bottom - tail_sway)
                ]

            pygame.draw.ellipse(fish_surf, fish['color'], body_rect)
            pygame.draw.polygon(fish_surf, fish['color'], tail_points)
            pygame.draw.ellipse(fish_surf, DARK_BROWN, body_rect, 1)
            pygame.draw.polygon(fish_surf, DARK_BROWN, tail_points, 1)

            eye_x = body_rect.right - 7 if not is_flipped else body_rect.left + 7
            eye_y = body_rect.centery - 4
            pygame.draw.circle(fish_surf, WHITE, (eye_x, eye_y), 4)
            pygame.draw.circle(fish_surf, BLACK, (eye_x, eye_y), 2)

            mouth_x = body_rect.right if not is_flipped else body_rect.left
            mouth_y = body_rect.centery + 3

            if fish['state'] == 'eating' and (fish['eat_timer'] // 2) % 2 == 0:
                 pygame.draw.circle(fish_surf, BLACK, (mouth_x + (3 if not is_flipped else -3), mouth_y), 3)
            else:
                 pygame.draw.line(fish_surf, BLACK, (mouth_x, mouth_y), (mouth_x + (4 if not is_flipped else -4), mouth_y), 1)

            final_fish_surf = fish_surf
            if is_flipped:
                 final_fish_surf = pygame.transform.flip(fish_surf, True, False)

            blit_x = fish['x'] - 10 if not is_flipped else fish['x'] - 5
            self.screen.blit(final_fish_surf, (blit_x, fish['y']))
    ...
    def draw(self):
        for y_pos in range(SCREEN_HEIGHT):
            r_val = int(255 - (y_pos / SCREEN_HEIGHT) * 60); g_val = int(250 - (y_pos / SCREEN_HEIGHT) * 55)
            b_val = int(245 - (y_pos / SCREEN_HEIGHT) * 50)
            color = (max(0, min(255, r_val)), max(0, min(255, g_val)), max(0, min(255, b_val)))
            pygame.draw.line(self.screen, color, (0, y_pos), (SCREEN_WIDTH, y_pos))
